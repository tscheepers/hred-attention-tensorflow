...
def gru_layer_with_state_reset(h_prev, x, name='gru', x_dim=512, y_dim=1024):
    """
    ...
    with tf.variable_scope(name):
        ...
        with tf.variable_scope('state_start'):
            W = tf.get_variable(name='weight', shape=(x_dim, y_dim), initializer=tf.truncated_normal_initializer())
            b = tf.get_variable(name='bias', shape=(y_dim,), initializer=tf.constant_initializer(0.0))

            h_prev_state = retain_vector * h_prev + (1 - retain_vector) * tf.tanh(tf.matmul(x, W) + b)

        h = _gru_layer_with_state(h_prev_state, x, state, 'gru', x_dim, y_dim)
...
def output_layer(h, x, name='output', x_dim=256, y_dim=512, h_dim=512):
    ...
    with tf.variable_scope(name):
        Wh = tf.get_variable(name='weight_hidden', shape=(x_dim, h_dim), initializer=tf.truncated_normal_initializer())
        Wi = tf.get_variable(name='weight_input', shape=(x_dim, y_dim), initializer=tf.truncated_normal_initializer())
        b = tf.get_variable(name='bias_input', shape=(y_dim,), initializer=tf.truncated_normal_initializer())
...
def softmax_layer(x, name='softmax', x_dim=512, y_dim=90004):
    ...
    with tf.variable_scope(name):
        ...
        W = tf.get_variable(name='weight', shape=(x_dim, y_dim), initializer=tf.truncated_normal_initializer())
        b = tf.get_variable(name='bias', shape=(y_dim,), initializer=tf.truncated_normal_initializer())
...
def _gru_layer(h_prev, x, name='gru', x_dim=256, y_dim=512):
    ...
    with tf.variable_scope(name):
        ...
        with tf.variable_scope('reset_gate'):
            ...
            Wi_r = tf.get_variable(name='weight_input', shape=(x_dim, y_dim), initializer=tf.truncated_normal_initializer())
        ...
        with tf.variable_scope('update_gate'):
            Wi_z = tf.get_variable(name='weight_input', shape=(x_dim, y_dim), initializer=tf.truncated_normal_initializer())
        ...
        with tf.variable_scope('candidate_update'):
            Wi_h_tilde = tf.get_variable(name='weight_input', shape=(x_dim, y_dim), initializer=tf.truncated_normal_initializer())
...
def _gru_layer_with_state(h_prev, x, state, name='gru', x_dim=512, y_dim=1024):
    """
    Used for decoder
    """
    ...
    with tf.variable_scope(name):
        ...
        with tf.variable_scope('reset_gate'):
            Wi_r = tf.get_variable(name='weight_input', shape=(x_dim, y_dim), initializer=tf.truncated_normal_initializer())
            Wh_r = tf.get_variable(name='weight_hidden', shape=(y_dim, y_dim), initializer=orthogonal_initializer())
            Ws_r = tf.get_variable(name='weight_state', shape=(x_dim, y_dim), initializer=tf.truncated_normal_initializer())
            b_r = tf.get_variable(name='bias', shape=(y_dim,), initializer=tf.constant_initializer(0.0))
        ...
        with tf.variable_scope('candidate_update'):
            Wi_h_tilde = tf.get_variable(name='weight_input', shape=(x_dim, y_dim), initializer=tf.truncated_normal_initializer())
            Wh_h_tilde = tf.get_variable(name='weight_hidden', shape=(y_dim, y_dim), initializer=orthogonal_initializer())
            Ws_h_tilde = tf.get_variable(name='weight_state', shape=(x_dim, y_dim), initializer=tf.truncated_normal_initializer())
            b_h_tilde = tf.get_variable(name='bias', shape=(y_dim,), initializer=tf.constant_initializer(0.0))
