...
class Processor:
    ...
    def process_gz_files(self):
        ...
        with open(sess_outfile_bg, 'w') as bg_sess, open(sess_outfile_tr, 'w') as tr_sess, \
            ...
            print("INFO - total sessions %d, unusable after preprocessing %d" % (session_id, sessions_unusable))
            print("INFO - total queries %d, unusable after preprocessing %d" % (total_queries, queries_unusable))
...
if __name__ == '__main__':
    ...
    p.translate_words_to_indices(final_proc_options=['tr', 'val', 'test'])
